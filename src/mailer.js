 ...
 *
 * Uses Resend's HTTPS API (not SMTP) — this works from any host, including
 * ones like Railway's Hobby plan that block outbound SMTP ports.
 */
async function sendDeepLinkEmail(toEmail, deepLink) {
  const { error } = await resend.emails.send({
    from: MAIL_FROM,
    to: toEmail,
    subject: 'הקישור שלך ל-Cheeky בטלגרם',
    html: `
      <p>תודה על התמיכה! 🎉</p>
